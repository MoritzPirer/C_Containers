...
VecStatus vec_get_unique(vec_t *self, vec_ordering_t vec_ordering, vec_t *unique)
{
    ...
    VecStatus result = vec_init(unique, 0, self->item_size);
    if (result != VEC_OK)
        return result;
...
size_t skipDuplicates(vec_t *self, size_t index, vec_ordering_t ordering)
{
    ...
    while (next < self->size_used && ordering(
                                    internal_vecNThElement(self, index),
                                    internal_vecNThElement(self, next)) == 0)
    {
        index = next;
        next++;
    }
...
VecStatus vec_get_intersection(vec_t *left, vec_t *right,
                                vec_ordering_t vec_ordering, vec_t *intersection)
{
    ...
    VecStatus result = vec_init(intersection, 0, left->item_size);
    if (result != VEC_OK)
        return result;
...
VecStatus vec_get_union(vec_t *left, vec_t *right,
                         vec_ordering_t vec_ordering, vec_t *union_elements)
{
    ...
    VecStatus result = vec_init(union_elements, 0, left->item_size);
    if (result != VEC_OK)
        return result;
